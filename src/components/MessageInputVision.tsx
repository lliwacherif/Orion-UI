...
const MessageInputVision: React.FC<MessageInputVisionProps> = ({ onSendMessage, disabled = false, hasMessages = false }) => {
  ...
  const handleSend = () => {
    ...
    alert(language === 'en'
      ? '🚧 OCR not yet implemented\n\nImage analysis with AI vision is coming soon!'
      : '🚧 OCR pas encore implémenté\n\nL\'analyse d\'images par IA arrive bientôt!');
        ...
        <>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex-shrink-0 bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-3 rounded-lg hover:from-purple-700 hover:to-pink-700 transition disabled:opacity-50 disabled:cursor-not-allowed font-medium"
            >
          ...
          <p className="text-xs text-gray-500 mt-2 text-center">
            {language === 'en'
              ? '⚠️ Image analysis coming soon - OCR not yet implemented'
              : '⚠️ Analyse d\'images bientôt disponible - OCR pas encore implémenté'}
          </p>
        </>
      ) : (
